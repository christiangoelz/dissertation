...
% applied in this work. Basically, DMD is an algorithm that allows to approximate the relation of all signals in pairs of consecutive time instances of a measurement X ∈ Rn×m , where n denotes sensors and m denotes measurement points at time t. With DMD it is possible to decompose X to extract spatio-temporal patterns of coherent brain activation (Brunton et al., 2016). To increase the number of calculated DMD modes and thus improve the approximation accuracy we used the delay embedding technique as applied by Brunton et al. (2016); stacking depth h = 4 was selected. Since the DMD algorithm takes advantage of the Singular Value Decomposition (SVD), the optimal rank r of the input matrix and therefor the number of SVD modes was determined using the method of Donoho and Gavish (2014). The decomposed spatial patterns are organized in a matrix Φ ∈ Chn×r , containing all DMD modes (i.e., spatial patterns) and a diagonal matrix Λ ∈ Cr ×r, containing the corresponding DMD eigenvalues. From this, the original measurement X can be approximated by the model:
% X̂ = Φ exp(Ω t)z, where Ω contains the continuous eigenvalues transformed with  = log(3)/1t .The variable t denotes time whereby ∆t = 0.005 s and z is
% computed from the first data point x1 of X with x1 = Φ z. From
% Ω we calculated the frequencies in cycles per second (Hz) f = |imag()/2π|for all DMD modes. The optimal stacking depth h as well as window size and overlap were determined by an error analysis of the data of
% five randomly selected subjects. As unique spatial information is present in the first n rows of a mode, we used Φ ∈ Cn×r .
...
% Approach learn a representation from EEG data by leveraging tools from dimensionality reduction 
% \subsubsection{Dimensionality reduction methods}
% As source separation problem 
% X = WS 
...
% Dimensionality reduction for visualizing EEG data 
% - PCA 
% - UMAP
...
% How to get features
% DMD: 
% The formula for the xDAWN algorithm is as follows:

% X_D = W^T * X

% Where:

%     X_D is the filtered EEG data (dimension: channels x time points)
%     W is the xDAWN spatial filter matrix (dimension: channels x channels)
%     X is the original raw EEG data (dimension: channels x time points)
%     ^T denotes matrix transpose operation
...
%     W is the xDAWN spatial filter matrix (dimension: channels x channels)
%     P is a diagonal matrix containing the eigenvalues of C in descending order (dimension: channels x channels)
%     U is the matrix of eigenvectors of C corresponding to the eigenvalues in P (dimension: channels x channels)
%     ^(-1/2) denotes the inverse square root of a matrix, which is computed by taking the inverse square root of each diagonal element of P

% The matrix W applies a linear transformation to the original EEG data X, as shown in the formula X_D = W^T * X, where X_D is the filtered EEG data.

% This formula allows the xDAWN algorithm to spatially filter the EEG data by projecting it onto the selected eigenvectors (U) and scaling the contribution of each channel by the corresponding eigenvalue (P). The resulting W matrix represents the set of xDAWN spatial filters that capture the most discriminative information in the EEG data.


% By applying the xDAWN algorithm, you can obtain a set of spatial filters that emphasize the relevant EEG activity while suppressing noise and artifacts, leading to improved signal quality for further analysis or classification tasks.
